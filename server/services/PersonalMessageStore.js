...
class PersonalMessageStore {
  async saveMessage(fromUserId, toUserId, message, timestamp) {
    try {
      const result = await pgPool.query(
        `INSERT INTO personal_messages (from_user_id, to_user_id, message, timestamp, delivered, read, created_at)
  ...
  async markRead(messageIds) {
    ...
    try {
      await pgPool.query(
        `UPDATE personal_messages SET read = true WHERE id = ANY($1)`,
        [messageIds]
      );
    } catch (error) {
